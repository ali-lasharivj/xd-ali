...
conn.ev.on('messages.upsert', async (msg) => {
 ...
 //=============WELCOME================= 
  ...
  const dave = ['923003588997'];  // اینجا یک آرایه قرار می‌دهیم
...
if (!isReact && config.CUSTOM_REACT === 'true') {
  ...
  app.get("/", (req, res) => {
    res.send(`
      ...
      <body>
        <div class="card">
          <h1><span class="status-dot"></span> ALI MD IS RUNNING</h1>
          <p>ALI IS MY OFC OWNER.</p>
        </div>
      </body>
      </html>
    `);
  });
  app.listen(port, () => console.log(`ali-md Server listening on port http://localhost:${port}`));
  setTimeout(() => {
  connectToWA()
  }, 4000);
